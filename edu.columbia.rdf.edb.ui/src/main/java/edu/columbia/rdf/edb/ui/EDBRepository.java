...
public class EDBRepository extends CacheRepository {
	...
	@Override
	public List<Sample> searchSamples(List<SearchStackElement<Sample>> searchStack,
			Path path) {
		return null;
	}
	...
	public List<Sample> searchSamples(String query, 
		...
		UrlBuilder url = getSearchSamplesUrl()
	...
	public List<Sample> parseSampleJson(Json json, SearchMode searchMode) {
		...
		for (int i = 0; i < json.size(); ++i) {
			...
			Experiment experiment = 
					mExperiments.get(sampleJSON.get(EDB.HEADING_EXPERIMENT).getAsInt());

			Type expression = 
					mDataTypes.get(sampleJSON.get(EDB.HEADING_TYPE).getAsInt());

			Species species = 
					mSpecies.get(sampleJSON.get(EDB.HEADING_ORGANISM).getAsInt());
			...
			if (stateType != null) {
				...
				state = SampleState.UNLOCKED;
			}
			 */

			Sample sample = new SampleAutoCache(sampleId,
					experiment,
					expression,
					name,
					species,
			...
			if (tagsJson != null) {
				for (int j = 0; j < tagsJson.size(); ++j) {
					...
					SampleTag sampleTag = new SampleTag(id,
							field,
	...
	private static List<FileDescriptor> getFiles(URL url, boolean showDirs) throws IOException {
		...
		for (int j = 0; j < json.size(); ++j) {
			...
			FileDescriptor file = new FileDescriptor(fileJSON.getAsInt(EDB.HEADING_ID),
					fileName,
					FileType.parse(fileJSON.get(EDB.HEADING_TYPE).getAsInt()),
					date);
	...
	/*
	private void cacheArrayDesign(Sample sample) throws IOException {
		URL url = new UrlBuilder(sampleUrl).addPath(sample.getUid()).addPath("gep").addPath("array").toUrl();

		JsonValue json = mJsonParser.parse(url);

		for (int i = 0; i < json.size(); ++i) {
			JsonValue arrayJSON = json.get(i);

			//String id = arrayJSON.get("id").getAsString();
			String name = arrayJSON.get("name").getAsString();
			String assay = arrayJSON.get("assay_name").getAsString();
			String provider = arrayJSON.get("provider_name").getAsString();
	...
	/*
	@Override
	public List<Peaks> searchChipSeqPeaks(Sample sample) throws ParseException, IOException, ParseException, java.text.ParseException {
		URL url = getChipSeqPeaksUrl(sample).toUrl();
		...
		for (int i = 0; i < json.size(); ++i) {
			...
			String peakCallerParameters = peaksJson.get("peak_caller_parameters").getAsString();

			SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
			Date date = formatter.parse(peaksJson.get("release_date").getAsString());

			Peaks peaks = new Peaks(id, name, genome, readLength, peakCaller, peakCallerParameters, date);

			Json locationsJson = peaksJson.get("peak_locations");

			for (int j = 0; j < locationsJson.size(); ++j) {
				Json locationJson = locationsJson.get(j);

				UCSCTrackRegion r = new UCSCTrackRegion(Chromosome.parse(locationJson.get("c").getAsString()),
						locationJson.get("s").getAsInt(),
						locationJson.get("e").getAsInt());
