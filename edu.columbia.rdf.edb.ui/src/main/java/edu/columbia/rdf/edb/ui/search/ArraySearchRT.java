...
public class ArraySearchRT {
	 ...
	 */
	/*
    public List<Experiment> searchExperiments(SearchStackElementCategory[] searchStack) {
    	Collection<Sample> samples = searchSamples(searchStack);
	 ...
	 * @throws IOException 
	 */
	public List<Sample> searchSamples(Deque<SearchStackElementCategory> searchStack,
		...
		// If no search criteria are specified or the search is an empty string, return all experiments.

		Repository store = RepositoryService.getInstance().getRepository();


		if (searchStack.size() == 0
				|| (searchStack.size() == 1 && searchStack.peek().getSearch() == null)) {
		...
		//System.err.println("ca search:" + op.type + " " + searchStack[searchStack.size() - 1).samples);
