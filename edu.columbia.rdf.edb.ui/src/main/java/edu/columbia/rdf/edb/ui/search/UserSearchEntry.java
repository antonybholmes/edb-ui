...
public class UserSearchEntry implements XmlRepresentation, Comparable<UserSearchEntry> {
	public static final SearchCategory DEFAULT_FIELD = 
			new SearchCategory("All Categories", new Path("/All"));
	
	public static final SearchStackOperator DEFAULT_BOOL_TERM = 
			SearchStackOperator.AND;
	 ...
	 */
	public UserSearchEntry(SearchStackOperator operator,
	...
	/**
	 * Returns an operator which must be one of the logical operators
	 * (and, or, xor etc).
	 * 
	 * @return
	 */
	...
	public static UserSearchEntry create(SearchStackOperator operator,
			SearchCategory field,
			String text) {
		return new UserSearchEntry(operator, 
				field, 
				text);
	}
