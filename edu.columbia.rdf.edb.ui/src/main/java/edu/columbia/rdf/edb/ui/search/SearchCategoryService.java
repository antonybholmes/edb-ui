...
/**
 * Represents all of the search fields available for the search
 * component.
 * 
...
public class SearchCategoryService implements Iterable<SearchCategoryGroup> {
	public static final File DEFAULT_SEARCH_CATEGORIES_XML_FILE = 
			new File("res/search.categories.xml");

	private final static Logger LOG = 
			LoggerFactory.getLogger(SearchCategoryService.class);
	
	private List<SearchCategoryGroup> groups = 
			new ArrayList<SearchCategoryGroup>();
	
	private Map<String, SearchCategoryGroup> groupMap = 
			new HashMap<String, SearchCategoryGroup>();
	
	private Map<String, SearchCategory> categoryMap = 
			new HashMap<String, SearchCategory>();
	
	private Map<Path, SearchCategory> pathMap = 
			new HashMap<Path, SearchCategory>();

	private static SearchCategoryService instance =
			new SearchCategoryService();
